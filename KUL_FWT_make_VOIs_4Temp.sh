...
function PD25_lab_gen {
    ...
    if [[ ! ${srch_PD25Ls} ]]; then
        ...
        for hl in ${!PD25_vals_LT[@]}; do
            ...
            task_in="mrcalc -force -datatype uint16 -force -nthreads 1 ${PD25_in_FOD} ${MSBP_csf_mask_binv} -mult ${PD25_vals_LT[$hl]} \
        ...
        for hr in ${!PD25_vals_RT[@]}; do
            ...
            task_in="mrcalc -force -datatype uint16 -force -nthreads 1 ${PD25_in_FOD} ${MSBP_csf_mask_binv} -mult ${PD25_vals_RT[$hr]} \
...
FOD2MS_str="${prep_d}/fod_2_UKBB_vMS_${subj}${ses_str}"
...
if [[ -z ${srch_pt1_done} ]]; then
    ...
    if [[ ! -f ${T1_BM_in_FOD_minCSF} ]]; then
        ...
        if [[ ! -f "${FOD2MS_str}_template.nii.gz" ]]; then

            task_in="antsIntermodalityIntrasubject.sh -d 3 -i ${temp_fod1} -r ${subj_FS_brain} -x ${subj_FS_bm} -t 3 -w ${prep_d}/MS_2_UKBB_${subj}_ -T ${UKBB_temp} -o ${FOD2MS_str}_"

            task_exec

        else

            echo "ANTs intermodality registration of FOD to MSBP T1 already done, skipping "  | tee -a ${prep_log2}

        fi

        task_in="antsApplyTransforms -d 3 -i ${subj_MS_brain} -o ${sub_T1inFOD} -r ${temp_fod1} -t [${FOD2MS_str}_0GenericAffine.mat,1] -t ${FOD2FS_str}_1InverseWarp.nii.gz"
        ...
        task_in="antsApplyTransforms -d 3 -i ${subj_MS_bm} -o ${T1_brain_mask_in_FOD} -r ${temp_fod1} -t [${FOD2MS_str}_0GenericAffine.mat,1] -t ${FOD2MS_str}_1InverseWarp.nii.gz -n multilabel"

        task_exec

        task_in="antsApplyTransforms -d 3 -i ${subj_MS_sc3} -o ${subj_MSsc3_in_FOD} -r ${temp_fod1} -t [${FOD2MS_str}_0GenericAffine.mat,1] -t ${FOD2MS_str}_1InverseWarp.nii.gz -n multilabel"
        ...
        task_in="antsApplyTransforms -d 3 -i ${subj_aparc_nii} -o ${subj_aparc_in_FOD} -r ${temp_fod1} -t [${FOD2MS_str}_0GenericAffine.mat,1] -t ${FOD2MS_str}_1InverseWarp.nii.gz -n multilabel" 

        task_exec

        task_in="antsApplyTransforms -d 3 -i ${subj_aseg_nii} -o ${subj_aseg_in_FOD} -r ${temp_fod1} -t [${FOD2MS_str}_0GenericAffine.mat,1] -t ${FOD2MS_str}_1InverseWarp.nii.gz -n multilabel" 
        ...
        ${T1_brain_mask_in_FOD} -sub -1 -eq 0.9 -ge ${MSBP_csf_mask} -force && fslmaths ${MSBP_csf_mask} -binv ${MSBP_csf_mask_binv} \
        && mrcalc -force -datatype uint16 -nthreads ${ncpu} ${T1_brain_mask_in_FOD} ${MSBP_csf_mask_binv} -mult ${T1_BM_in_FOD_minCSF}"
...
if [[ -z ${srch_pt2_done} ]]; then
    ...
    # LT - RT : "248" "124" \
    task_in="mrcalc -force -datatype uint16 -quiet -nthreads ${ncpu} `mrcalc -datatype uint16 -force -quiet ${subj_MSsc3_in_FOD} 248 -eq - ` \
    `mrcalc -force -datatype uint16 -quiet ${subj_MSsc3_in_FOD} 124 -eq - ` -add 0.5 -gt \
    ...
    if [[ -z ${srch_dotdones[$q]} ]]; then
        ...
        if [[ ! ${tck_list[$q]} == *"none"* ]]; then
            ...
            if [[ ${tck_list[$q]} == "CST_LT" ]]; then
                ...
                SRF_LT_incs1_Ls=("Ling3_MSBP_LT" "Fusi2_MSBP_LT" "Fusi3_MSBP_LT" "Fusi4_GMWM_LT_custom");

                SRF_LT_incs1_Is=("207" "209" "210" "1");
                ...
                SRF_RT_incs1_Ls=("Ling3_MSBP_RT" "Fusi2_MSBP_RT" "Fusi3_MSBP_RT" "Fusi4_GMWM_RT_custom");

                SRF_RT_incs1_Is=("83" "86" "87" "1");
                ...
                ThR_OCD_DBS_LT_excs_Ls=("CC_allr_custom" "BStem_FS" "AC_midline_MAN" \
                "hypothal_bildil_excr_custom" "Temp_lobeGM_LT" "Putamen_GM_LT_FS" \
                "Caudate_GM_LT_FS" "PD25_Pulvi_LT_custom" "Fornix_Fx" \
                ...
                ThR_OCD_DBS_RT_excs_Ls=("CC_allr_custom" "BStem_FS" "AC_midline_MAN" \
                "hypothal_bildil_excr_custom" "Temp_lobeGM_RT" "Putamen_GM_RT_FS" \
                "Caudate_GM_RT_FS" "PD25_Pulvi_RT_custom" "Fornix_Fx" \
                ...
                Post_Comm_excs_Is=("1" "251" "250" "1" "85" "248" "124" "1006" "3006" "2006" "4006");
            ...
            elif [[ ${tck_list[$q]} == "CC_SMAR_2contM1LT_Comm" ]]; then

                CC_SMAR_2contM1LT_Comm_incs1_Ls=("CC_mid_FS" "CC_isth_FS");

                CC_SMAR_2contM1LT_Comm_incs1_Is=("253" "252");

                tck_VOIs_2seg="${tck_list[$q]}_incs1" && make_VOIs

                CC_SMAR_2contM1LT_Comm_incs2_Ls=("M1_GM_FS_LT");

                CC_SMAR_2contM1LT_Comm_incs2_Is=("1024");

                tck_VOIs_2seg="${tck_list[$q]}_incs2" && make_VOIs

                CC_SMAR_2contM1LT_Comm_incs3_Ls=("SFG5_RT_MSBP" "SFG6_RT_MSBP" "SFG7_RT_MSBP" "SFG8_RT_MSBP");

                CC_SMAR_2contM1LT_Comm_incs3_Is=("24" "25" "26" "27");

                tck_VOIs_2seg="${tck_list[$q]}_incs3" && make_VOIs

                # should probably add an AC_min_Fx VOI as well
                CC_SMAR_2contM1LT_Comm_excs_Ls=("AC_MAN" "PC_MAN" "CC_genu_FS" "CC_ant_FS" "CC_splen_FS" "Fornix_Fx" "BStem_FS" \
                "Cing_lobeGM_LT" "Cing_lobeGM_RT" "SegWM_LT_ALIC_custom" "SegWM_RT_ALIC_custom" "vDC_FS_LT" "vDC_FS_RT");

                CC_SMAR_2contM1LT_Comm_excs_Is=("1" "2" "255" "254" "251" "250" "16" \
                "1003" "2003" "1" "1" "28" "60");

                tck_VOIs_2seg="${tck_list[$q]}_excs" && make_VOIs

            elif [[ ${tck_list[$q]} == "CC_SMAL_2contM1RT_Comm" ]]; then

                CC_SMAL_2contM1RT_Comm_incs1_Ls=("CC_mid_FS" "CC_isth_FS");

                CC_SMAL_2contM1RT_Comm_incs1_Is=("253" "252");

                tck_VOIs_2seg="${tck_list[$q]}_incs1" && make_VOIs

                CC_SMAL_2contM1RT_Comm_incs2_Ls=("M1_GM_FS_RT");

                CC_SMAL_2contM1RT_Comm_incs2_Is=("2024");

                tck_VOIs_2seg="${tck_list[$q]}_incs2" && make_VOIs

                CC_SMAL_2contM1RT_Comm_incs3_Ls=("SFG5_LT_MSBP" "SFG6_LT_MSBP" "SFG7_LT_MSBP" "SFG8_LT_MSBP");

                CC_SMAL_2contM1RT_Comm_incs3_Is=("148" "149" "150" "151");

                tck_VOIs_2seg="${tck_list[$q]}_incs3" && make_VOIs

                # should probably add an AC_min_Fx VOI as well
                CC_SMAL_2contM1RT_Comm_excs_Ls=("AC_MAN" "PC_MAN" "CC_genu_FS" "CC_ant_FS" "CC_splen_FS" "Fornix_Fx" "BStem_FS" \
                "Cing_lobeGM_LT" "Cing_lobeGM_RT" "SegWM_LT_ALIC_custom" "SegWM_RT_ALIC_custom" "vDC_FS_LT" "vDC_FS_RT");

                CC_SMAL_2contM1RT_Comm_excs_Is=("1" "2" "255" "254" "251" "250" "16" \
                "1003" "2003" "1" "1" "28" "60");
