        ...
        if [[ ! -f "${TCK_out}/${TCK_2_make}_fin_map_${T}_${algo_f}_inMNI.nii.gz" ]]; then
            ...
            if [[ ! ${TCK_2_make} == "O"* ]]; then
                ...
                if [[ ! -f ${tck_filt1} ]]; then
                    ...
                    task_in="scil_filter_tractogram.py -f --reference ${temp_fod1} ${drawn_incs_str} ${drawn_excs_str} ${auto_X_f} ${tck_init} ${tck_filt1}"
                ...
                if [[ -f ${tck_filt1} ]] && [[ ! -f ${tck_filt5} ]] && [[ ${count2} -gt 10 ]]; then
                    ...
                    task_in="scil_detect_streamlines_loops.py -f --qb --reference ${temp_fod1} ${tck_filt1} ${tck_filt2}"
                    ...
                    task_in="scil_smooth_streamlines.py -f --gaussian 5 --reference ${temp_fod1} ${tck_filt3} ${tck_filt5}"
                ...
                if [[ ! -f ${tck_filt1} ]]; then
                    ...
                    task_in="KUL_FWT_FBC_4TCKs.py -i ${tck_init} -r ${temp_fod1} -o ${tck_filt1}"
    ...
    if [[ "${Q_flag}" -eq 1 ]]; then
        ...
        if [[ -f "${TCK_out}/${TCK_2_make}_fin_${T}_${algo_f}.tck" ]] && [[ ! -f "${TCK_out}/QQ/${TCK_2_make}_fin_${T}_${algo_f}_QQ_done.done" ]]; then 
            ...
            if [[ "${#TCK_Is_MNI[@]}" -eq 2 ]]; then
                ...
                # task_in="fslmaths ${lego_1} -mul ${TCK_Is_MNI[0]} -bin -save ${lego_1a} -restart ${lego_1} -mul ${TCK_Is_MNI[-1]} -bin ${lego_1b}"

                # task_exec

                # task_in="fslmaths ${lego_2} -mul ${TCK_Is_MNI[0]} -bin -save ${lego_2a} -restart ${lego_2} -mul ${TCK_Is_MNI[-1]} -bin ${lego_2b}"
            ...
            mets_fs=("${subj_FOD_MNI}" "${subj_ADC_MNI}" "${subj_AD_MNI}" "${subj_RD_MNI}" "${TCKs_tdi}" "${TCKs_length}" "${TCKs_curve}")
...
subj_FOD_MNI="${prep_d}/FOD_MNI.nii.gz"
...
if [[ -z "${ROIs_d}/Part1.done" ]] && [[ -z "${ROIs_d}/Part2.done" ]]; then
    ...
    if [[ ${T_app} -eq 2 ]]; then
        ...
        if [[ -z ${WB_tck_srch} ]]; then
            ...
            echo " Whole brain tractogram found, skipping to segmentation " | tee -a ${prep_log2}
