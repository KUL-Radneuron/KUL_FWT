...
- **Bug fixes**:
  - Fixed race condition in `make_VOIs.sh` where background custom VOI creation jobs (hypothal_vDC, Bs_Pons, etc.) were not guaranteed to complete before the bundle loop read them; a `wait` call was added at the correct synchronisation point.
  - Fixed `count_checker` string comparison in `make_TCKs.sh` (`== 0` → `-eq 0`) to correctly handle leading whitespace from `cut`.
  - Fixed spurious label `”14”` in `SLF_IIv_RT_incs1_Is`.
  - Added fallback in `make_TCKs.sh` and `make_TCKs_4Temp.sh`: if recobundles crashes silently and `filt5` is never created, `filt4` is promoted to `filt5` so downstream steps are not blocked.
